<div class="mt-35">
    <div class="course-reviews-box row align-items-center">
        <div class="col-3 text-center">
            <div class="reviews-rate font-36 font-weight-bold text-primary">{{ $user->getInstructorRate() }}</div>
            ...
            <div class="text-center">
                ...
                <div class="mt-15">{{ $user->instructorReviews->pluck('creator_id')->count() }}
                    {{ trans('product.reviews') }}</div>
            </div>
        </div>
        ...
        <div class="col-9">
            <div class="d-flex align-items-center">
                <div class="progress course-progress rounded-sm">
                    <span class="progress-bar rounded-sm"
                        style="width: {{ ($user->instructorReviews->avg('content_quality') / 5) * 100 }}%"></span>
                </div>
                <span class="ml-15 font-14 text-gray text-left">{{ trans('product.content_quality') }}
                    ({{ $user->instructorReviews->count() > 0 ? round($user->instructorReviews->avg('content_quality'), 1) : 0 }})</span>
            </div>

            <div class="mt-25 d-flex align-items-center">
                <div class="progress course-progress rounded-sm">
                    <span class="progress-bar rounded-sm"
                        style="width: {{ ($user->instructorReviews->avg('instructor_skills') / 5) * 100 }}%"></span>
                </div>
                <span class="ml-15 font-14 text-gray text-left">{{ trans('product.instructor_skills') }}
                    ({{ $user->instructorReviews->count() > 0 ? round($user->instructorReviews->avg('instructor_skills'), 1) : 0 }})</span>
            </div>

            <div class="mt-25 d-flex align-items-center">
                <div class="progress course-progress rounded-sm">
                    <span class="progress-bar rounded-sm"
                        style="width: {{ ($user->instructorReviews->avg('purchase_worth') / 5) * 100 }}%"></span>
                </div>
                <span class="ml-15 font-14 text-gray text-left">{{ trans('product.purchase_worth') }}
                    ({{ $user->instructorReviews->count() > 0 ? round($user->instructorReviews->avg('purchase_worth'), 1) : 0 }})</span>
            </div>

            <div class="mt-25 d-flex align-items-center">
                <div class="progress course-progress rounded-sm">
                    <span class="progress-bar rounded-sm"
                        style="width: {{ ($user->instructorReviews->avg('support_quality') / 5) * 100 }}%"></span>
                </div>
                <span class="ml-15 font-14 text-gray text-left">{{ trans('product.support_quality') }}
                    ({{ $user->instructorReviews->count() > 0 ? round($user->instructorReviews->avg('support_quality'), 1) : 0 }})</span>
            </div>
...
<section class="mt-40">
    <h2 class="section-title after-line">{{ trans('product.reviews') }}
        ({{ $user->instructorReviews->pluck('creator_id')->count() }})</h2>
    @if (isset($authUser))
        <form action="/reviews/store" class="mt-20" method="post">
            {{ csrf_field() }}
            <input type="hidden" name="creator_id" value="{{ $authUser->id }}" />
    ...
    @endif
    <div class="mt-45">
        @if ($user->instructorReviews->count() > 0)
            @foreach ($user->instructorReviews as $review)
                <div class="comments-card shadow-lg rounded-sm border px-20 py-15 mt-30"
                    data-address="/reviews/store-reply-comment" data-csrf="{{ csrf_token() }}"
                    data-id="{{ $review->id }}">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <div class="d-flex align-items-center">
                            <span
                                class="font-12 text-gray mr-10">{{ dateTimeFormat($review->created_at, 'j M Y | H:i') }}</span>
                            ...
                            <div class="btn-group dropdown table-actions">
                                ...
                                <div class="dropdown-menu">
                                    <a href="/reviews/store-reply-comment"
                                        class="webinar-actions d-block text-hover-primary reply-comment">{{ trans('panel.reply') }}</a>
                    ...
                    {{-- @if ($user->takenComments->count() > 0)
                      @foreach ($user->takenComments as $comment)
