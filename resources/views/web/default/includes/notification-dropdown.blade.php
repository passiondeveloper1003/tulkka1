        ...
        <div class="d-flex flex-column h-100">
            ...
            @if(!empty($unReadNotifications) and count($unReadNotifications))
                <div class="mt-10 navbar-notification-action">
                    <a href="/panel/notifications" class="btn btn-sm btn-primary btn-block">{{ trans('notification.all_notifications') }}</a>
                </div>
            @endif
            @if(!empty($unReadNotifications) and count($unReadNotifications))
                <div class="mt-10 navbar-notification-action">
                    <a @if(isset($authUser)) href="/panel/notifications/clear" @endif class="btn btn-sm btn-danger btn-block">{{ trans('notification.mark_readed') }}</a>
                </div>
            @endif
        </div>
    </div>
