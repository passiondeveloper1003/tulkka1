...
@section('content')
    <section>
        <div class="d-flex flex-row justify-content-between">
            <h2 class="font-weight-normal font-18">{{ trans('panel.homework_stats') }}</h2>
        </div>
        <div class="activities-container mt-3">
            <div class="row row-40">
                <div class="col-12 col-md-6 col-xl-3 mb-20 mb-md-40 mb-xl-20">
                    <div class="d-flex justify-content-center stats-card">
                        <div class="d-flex flex-column align-items-center text-center mt-40">
                            ...
                            <strong class="font-36 font-weight-bold mt-20">{{ $homeworks->total() ?? 0 }}</strong>
                            <span class="font-16 text-dark-blue text-gray mt-10">{{ trans('quiz.homeworks') }}</span>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-6 col-xl-3 mb-20 mb-md-40 mb-xl-20">
                    <div class=" d-flex justify-content-center stats-card">
                        <div class="d-flex flex-column align-items-center text-center mt-40">
                            ...
                            <span class="font-16 text-dark-blue text-gray mt-10">{{ trans('public.homework_waiting') }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section>
        <h2 class="font-18 font-weight-normal">{{ trans('quiz.filter_homeworks') }}</h2>
    ...
    <section class="mt-35">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="font-18 font-weight-normal">{{ trans('quiz.homeworks') }}</h2>
            ...
            <form action="/panel/quizzes" method="get" class="">
                <div
                    ...
                    <label class="mb-0 mr-10 cursor-pointer text-gray font-14 font-weight-500"
                        for="activeQuizzesSwitch">{{ trans('quiz.show_only_waiting_homeworks') }}</label>
                            ...
                            <table class="table text-center custom-table">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($homeworks as $homework)
                                        <tr>
                                            <td class="text-left">
                                                <span class="d-block">{{ $homework->title }}</span>
                                            </td>
                                            </td>
                                            <td class="text-center align-middle">
                                                {{ !$user->isTeacher() ? $homework->teacher->full_name : $homework->student->full_name }}
                                            </td>
                                            <td class="text-center align-middle">
                                                @switch($homework->status)
                                                    ...
                                                    @case('active')
                                                        <span
                                                            class="text-warning font-weight-500">{{ trans('public.open') }}</span>
                                                    @break
                                            ...
                                            {{-- <td class="text-center align-middle">
                                                {{ $homework->result ?? 'Not Evaluated' }}
                                            </td> --}}
                                            <td class="text-center align-middle">
                                                {{ \Carbon\Carbon::parse($homework->created_at)->setTimezone($user->time_zone)->toDateTimeString() }}
                                            </td>
                                            <td class="text-center align-middle">

                                                <a href="/panel/homeworks/{{ $homework->id }}/details"
                                                    class="webinar-actions  mt-10 text-primary">{{ trans('public.open_detail') }}</a>
                                                <a href="/panel/homeworks/{{ $homework->id }}/delete"
        ...
        @else
            @include(getTemplate() . '.includes.no-result', [
                'file_name' => 'quiz.png',
                'title' => trans('quiz.homework_no_result'),
                'hint' => nl2br(trans('quiz.homework_no_result_hint')),
            ])
        @endif
